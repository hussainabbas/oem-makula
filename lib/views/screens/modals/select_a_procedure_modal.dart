...
class SelectProcedureModal {
  static void show(BuildContext context , GetProcedureTemplatesResponse? getProcedureTemplatesResponse, String workOrderId, VoidCallback callback) {
    showModalBottomSheet(
      ...
      builder: (context) {
        ...
        observerAttachProcedureToWorkOrder(AttachProcedureToWorkOrderResponse response) {
          context.showSuccessSnackBar(response.attachOwnOemProcedureToWorkOrder ?? "");
        }

        attachProcedureToWorkOrder(String workOrderId, String templateId) async {
          var isConnected = await isConnectedToNetwork();
          if (isConnected) {
            if (context.mounted) {
              context.showCustomDialog();
            }
            var result = await ProcedureViewModel().attachProcedureToWorkOrder(workOrderId, templateId);
            if (context.mounted) {
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Expanded(child: SelectProcedureExpansionTile(templatesResponse: getProcedureTemplatesResponse,)),
                        ...
                        child: CustomElevatedButton(
                            ...
                            onPressed: () {
                              if (procedureProvider.selectedProcedure != null) {
                                attachProcedureToWorkOrder(workOrderId, procedureProvider.selectedProcedure?.sId ?? "");
                                //Navigator.pop(context);
...
class SelectProcedureExpansionTile extends StatelessWidget {
  ...
  const SelectProcedureExpansionTile({super.key , required this.templatesResponse});
  ...
  Widget build(BuildContext context) {
    ...
    return ExpansionTile(
      ...
      children: [
        SizedBox(
          height: context.fullHeight(multiplier: 0.28),
          child: ListView.builder(
            itemCount: templatesResponse?.listOwnOemProcedureTemplates?.length,
            itemBuilder: (context, index) {
              return ListTile(
                onTap: () {
                  console("templatesResponse => ${templatesResponse?.listOwnOemProcedureTemplates?[index].sId}");
                  procedureProvider.updateSelectedProcedure(templatesResponse!.listOwnOemProcedureTemplates?[index]);
                ...
                title: TextView(
                    text: templatesResponse!.listOwnOemProcedureTemplates?[index].name ?? "",
                    textColor: Colors.black,
                    textFontWeight: FontWeight.normal,
                    fontSize: 12),
              );
