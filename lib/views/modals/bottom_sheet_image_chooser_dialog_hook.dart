...
import '../../main.dart';
import '../screens/dashboard/tabbar/tickets/details/procedure_viewmodel.dart';
import 'package:http/http.dart' as http;

class BottomSheetImageChooserHookModal {
  static void show(
    ...
    String procedureId,
    WidgetRef ref,
      Function(ImageRequestModel?) onSelect,
  ) {
    showModalBottomSheet(
        ...
        builder: (BuildContext context) {
          return SingleChildScrollView(
            ...
            final selectedImageState =
                ref.watch(selectedImageListProvider.notifier);
            void pickImageFromGallery() async {
              ...
              if (pickedFile != null && context.mounted) {
                ...
                ref
                    .watch(selectedImageListProvider.notifier)
                    .state
                    ?.add(pickedFile.path);
                setState(() {});
              }
            ...
            uploadImageOnBucket(
              ...
              if (response.statusCode == 200 && context.mounted) {
                    ...
                    // var listOwnOemProcedureTemplates = ref.watch(procedureTemplateProvider.notifier).state;

                    // formStateProvider.setUploadedImageList(safeSigns3Response, fieldName);
                    ...
                    // if (isAChildren) {
                    //   listOwnOemProcedureTemplates?.children?[parentIndex].children?[index].value.add(imageRequestModel);
                    // } else {
                    //   listOwnOemProcedureTemplates?.children?[parentIndex].value.add(imageRequestModel);
                    // }
                    // ref.read(procedureTemplateProvider.notifier).updateValue(listOwnOemProcedureTemplates);

                    onSelect(imageRequestModel);
                    ...
                    ref
                        .watch(
                        selectedImageListProvider.notifier)
                        .state
                        ?.remove(file.path);
            ...
            uploadImagesToS3() async {
              ...
              if (isConnected) {
                var list = ref.watch(selectedImageListProvider.notifier).state ?? [];
            ...
            Widget buildImageWrap(BuildContext context) {
              return StatefulBuilder(builder: (context, setState) {
                return StreamBuilder<List<String>?>(
                    stream: selectedImageState.stream,
